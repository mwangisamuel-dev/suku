...
class _MpesaScreenState extends State<MpesaScreen> {
  ...
  Future _loadRecentSms() async {
    setState(() => _loadingSms = true);
    final has = await SmsService.hasSmsPermission();
    if (!has) {
      await SmsService.requestSmsPermission();
    }
    final sms = await SmsService.getRecentMpesaSms();
    if (mounted)
      setState(() {
        _recentSms = sms;
        _loadingSms = false;
  ...
  void _parseSms(String sms) {
    ...
    if (firstMatch == null) {
      setState(() => _parsed = {'error': 'Could not find an amount in this SMS.'});
      return;
    }
    final amount = double.tryParse(firstMatch.group(1)!.replaceAll(',', '')) ?? 0;
    ...
    if (isIncome) {
      ...
      final toRegex = RegExp(r'(?:sent to|paid to) ([A-Z\s]+) (?:on|via)');
    ...
    if (lower.contains('kplc') || lower.contains('kenya power') || lower.contains('nairobi water')) {
    ...
    } else if (lower.contains('salary') || lower.contains('mshahara') || lower.contains('wages')) {
  ...
  Future<void> _save() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Row(
        children: [
          Icon(success ? Icons.check_circle_rounded : Icons.error_outline_rounded, color: Colors.white, size: 18),
          const SizedBox(width: 8),
          Text(success ? 'M-Pesa transaction saved!' : 'Failed to save. Try again.',
              style: GoogleFonts.plusJakartaSans(fontWeight: FontWeight.w600, color: Colors.white)),
        ],
      ),
      backgroundColor: success ? SukuColors.green : SukuColors.error,
        ...
        title: Row(
          children: [
            Container(
              ...
              child: const Icon(Icons.phone_android_rounded, color: SukuColors.green, size: 18),
            ),
            const SizedBox(width: 10),
            Text('M-Pesa Import', style: GoogleFonts.plusJakartaSans(fontSize: 18, fontWeight: FontWeight.w700)),
          ],
        ),
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: Row(
                children: [
                  const Icon(Icons.info_outline_rounded, color: SukuColors.green, size: 20),
                  const SizedBox(width: 10),
                  Expanded(
                    child: Text(
                      'Paste any M-Pesa SMS below. Suku will read it and save to your books.',
                      style: GoogleFonts.plusJakartaSans(fontSize: 13, color: SukuColors.greenDark, height: 1.5),
                    ),
                  ),
                ],
              ),
            ...
            TextField(
              ...
              decoration: InputDecoration(
                hintText: 'e.g. TGR76Y Confirmed. Ksh4,500 sent to NAIVAS...',
                hintStyle: GoogleFonts.plusJakartaSans(fontSize: 12, color: SukuColors.textHint),
                filled: true,
                ...
                Expanded(
                  child: SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      onPressed: () => _parseSms(_smsController.text),
            ...
            const SizedBox(height: 20),
            // Real M-Pesa SMS from inbox
            if (_loadingSms)
              const Center(child: CircularProgressIndicator(color: SukuColors.green))
            else if (_recentSms.isNotEmpty) ...[
              Text('Recent M-Pesa SMS',
                  style: GoogleFonts.plusJakartaSans(
                      fontSize: 14, fontWeight: FontWeight.w600, color: SukuColors.textPrimary)),
              const SizedBox(height: 10),
              ..._recentSms.take(5).map((sms) => GestureDetector(
                    onTap: () {
                      ...
                      child: Column(
                        ...
                        children: [
                          Row(
                            ...
                            children: [
                              Container(
                                ...
                                child: Text('M-PESA',
                                    style: GoogleFonts.plusJakartaSans(
                                        fontSize: 10, fontWeight: FontWeight.w700, color: SukuColors.green)),
                              ),
                              if (sms.date != null)
                                Text(
                                  DateFormat('d MMM, HH:mm').format(DateTime.fromMillisecondsSinceEpoch(sms.date!)),
                                  style: GoogleFonts.plusJakartaSans(fontSize: 10, color: SukuColors.textHint),
                                ),
                            ],
                          ),
                          ...
                          Text(
                            sms.body ?? '',
                            style:
                                GoogleFonts.plusJakartaSans(fontSize: 11, color: SukuColors.textSecondary, height: 1.5),
                            maxLines: 2,
            ...
            if (_parsed != null) ...[
              ...
              if (_parsed!.containsKey('error'))
                Container(
                  ...
                  child: Row(
                    children: [
                      const Icon(Icons.error_outline_rounded, color: SukuColors.error, size: 20),
                      const SizedBox(width: 8),
                      Expanded(
                        child: Text(_parsed!['error'] as String,
                ...
                const SectionHeader(title: 'Extracted Transaction'),
                const SizedBox(height: 12),
                Container(
                  ...
                  child: Column(
                    children: [
                      _row('Type', (_parsed!['type'] == TransactionType.income) ? 'Money In ↑' : 'Money Out ↓',
                          _parsed!['type'] == TransactionType.income ? SukuColors.green : SukuColors.error),
                      const Divider(height: 20, color: SukuColors.border),
                      _row('Amount', 'Ksh ${(_parsed!['amount'] as double).toStringAsFixed(0)}', SukuColors.textPrimary,
                          bold: true),
                      const Divider(height: 20, color: SukuColors.border),
                      _row('Vendor / From', _parsed!['vendor'] as String, SukuColors.textPrimary),
                      if (_parsed!['category'] != null) ...[
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            CategoryBadge(category: _parsed!['category'] as ExpenseCategory),
                          ],
                        ),
                      ],
                    ],
                ...
                SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    icon: _saving
                        ...
                        : Icon(_saved ? Icons.check_circle_rounded : Icons.save_rounded, size: 20),
                    label: Text(
                        _saving
                            ? 'Saving...'
                            : _saved
                    ...
                    style: ElevatedButton.styleFrom(
                      backgroundColor: _saved ? SukuColors.greenDark : SukuColors.green,
                      foregroundColor: Colors.white,
                      elevation: 0,
  ...
  Widget _row(String label, String value, Color valueColor, {bool bold = false}) {
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        Text(label, style: GoogleFonts.plusJakartaSans(fontSize: 13, color: SukuColors.textSecondary)),
        Text(value,
            style: GoogleFonts.plusJakartaSans(
                fontSize: bold ? 18 : 14,
